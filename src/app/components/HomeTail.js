...
import { headings } from "@/data/home";
 ...
 *
 * Three beats: the process (two tabbed drawing sets, data/process.js, which
 * replaced the two doors and the delegation strip on 2026-09-10), the close,
 * the footer. Proof lives on /work/ (reached from the flight's "See the
 * work" and the nav), not here.
 */

export default function HomeTail() {
  return (
    <section className="relative z-[45]" aria-label="How it gets built">
      ...
      <div className="bg-ink" data-scrim-from>
        <div className="mx-auto w-full max-w-6xl px-6 pt-4">
          <SectionHeading eyebrow={headings.process.eyebrow} title={headings.process.title}>
            {headings.process.body}
          </SectionHeading>
          <ProcessTabs />
          <CtaPanel {...startClose} />
          <SiteFooter />
